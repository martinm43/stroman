...
# for the model
futuredata = Game.select().where(
    Game.scheduled_date >= datetime.today() -
    timedelta(
        days=1)).order_by(
            Game.scheduled_date)
futuredata = [[g.away_team, g.away_runs, g.home_team, g.home_runs]
              for g in futuredata]
pastdata = Game.select().where(
    Game.scheduled_date < datetime.today() -
    timedelta(
        days=1)).order_by(
            Game.scheduled_date)
pastdata = [[g.away_team, g.away_runs, g.home_team, g.home_runs]
            for g in pastdata]
